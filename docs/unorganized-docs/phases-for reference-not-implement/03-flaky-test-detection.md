# Phase 03: Flaky Test Detection

> **Priority:** High | **Complexity:** Medium | **Dependencies:** Phase 01 (Test Signatures)
>
...
- Phase 01 completed (test_signature column exists)
...
// New interface for flakiness data
...
// Flakiness summary for dashboard
...
export async function getTestFlakiness(
  ...
  return results[0] as TestFlakinessHistory || null;
}
```

### 3. Create Flakiness API Route (`app/api/flakiness/route.ts`)
...
### 1. Create Flaky Badge Component (`components/dashboard/flaky-badge.tsx`)
...
import { Badge } from "@/components/ui/badge";
import { AlertTriangle, TrendingUp, FileCode } from "lucide-react";
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground text-sm">
            No flaky tests detected yet. Great job!
          </p>
        </CardContent>
      </Card>
    );
...
### 4. Update Test Detail Modal (`components/dashboard/test-detail-modal.tsx`)

Add FlakyBadge to test items that are flaky:

```tsx
// Import FlakyBadge
import { FlakyBadge } from "./flaky-badge";

// In the test item rendering, add:
{test.is_flaky && (
  <FlakyBadge
    flakinessRate={test.retry_count ? 100 : 0}
    showTooltip={false}
  />
)}
```

### 5. Add Flakiest Tests Card to Dashboard (`app/page.tsx`)

```tsx
import { FlakiestTestsCard } from "@/components/dashboard/flakiest-tests-card";

// In the dashboard layout, add the card:
...
- `lib/db.ts` - Add flakiness functions
- `components/dashboard/test-detail-modal.tsx` - Add FlakyBadge
...
ALTER TABLE test_results DROP COLUMN IF EXISTS is_flaky;
DROP TABLE IF EXISTS test_flakiness_history;
DROP INDEX IF EXISTS idx_test_results_flaky;
```

---

*Phase 03 Complete → Proceed to Phase 04: Failure Rate Metrics*
