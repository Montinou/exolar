...
3. Add failure rate trend to visualizations
4. Track failure volume (rate × frequency)
...
export interface DashboardMetrics {
  ...
  failure_volume: number;      // NEW: failure_rate × total_executions
}
...
export async function getDashboardMetrics(
  ...
  const result = await sql`
    ...
    SELECT
      ...
      CASE
        WHEN es.total_executions > 0
        THEN ROUND(es.failed::decimal / es.total_executions * 100 * es.total_executions / 100, 1)
        ELSE 0
      END as failure_volume
...
export function StatsCards({ metrics }: StatsCardsProps) {
  ...
  const cards = [
    ...
    },
    {
      ...
      icon: XCircle,
      description: `Volume: ${metrics.failure_volume.toFixed(1)}`,
...
### 2. Create Failure Rate Trend Chart (`components/dashboard/failure-rate-chart.tsx`)
...
export function FailureRateChart({ dateFrom, dateTo }: FailureRateChartProps) {
  ...
  const avgFailureRate =
    data.length > 0
      ? data.reduce((sum, d) => sum + d.failure_rate, 0) / data.length
      : 0;
                  ...
                  return (
                    <div className="bg-popover border rounded-lg p-2 shadow-lg">
                      ...
                      <p className="text-red-500">
                        Failure Rate: {data.failure_rate.toFixed(1)}%
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {data.failed_tests} / {data.total_tests} executions
                      </p>
                    </div>
                  );
                }
...
Add the FailureRateChart alongside existing charts:
...
### Modify:
- `lib/types.ts` - Add failure_rate and failure_volume to DashboardMetrics
- `lib/db.ts` - Update getDashboardMetrics, add getFailureTrendData
- `app/api/trends/route.ts` - Add type parameter for failure trends
- `components/dashboard/stats-cards.tsx` - Add failure rate card
- `app/page.tsx` - Add FailureRateChart

---

*Phase 04 Complete → Proceed to Phase 05: Error Pattern Grouping*
